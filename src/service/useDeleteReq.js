...
const useDeleteReq = () => {
  ...
  const deleteData = (path,data,onComplete, onError, reqParams) => {

    let apiPath = path.split("/");
    let pathName = apiPath[apiPath.length - 1];
      CustomAxios.delete(path,data,{
        ...reqParams,
      })
        .then((res) => {
          if (onComplete){ onComplete(res.data);
            logData = [{...viewData, 'module_name': pathName, 'payload': data, 'api': `/${pathName}`, 'response':res.data, 'status_code': res.status, 'error_details': '' }];
