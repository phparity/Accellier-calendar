...
const OrangeWorkflowButton = (props)  => {
  ...
  const embeddingModule = () => {
    ...
    if (window.location.href.match(/detail/g)) {
      vwname = 'detail'
      dataSet = {
        'detailsId' : detailsId[(detailsId.length - 1)] !== undefined ? detailsId[(detailsId.length - 1)] : null,
    ...
    setDash([]);
    if (module_name === "home") {
      axios.get("/" + tenantCname + "/api/embedding", {
        headers: {
          "Accept": "application/JSON",
          "Authorization": "Bearer " + localStorage.getItem('token')
        },
        ...
        .then((res) => {
          datafunc(res.data)
          logData = [{...viewData, 'module_name': module_name, 'api': `/${'embedding'}`, 'payload':{...dataSet}, 'response':res.data, 'status_code': res.status, 'error_details': ''}];
        ...
        .catch(err => {
          console.log(err) 
          logData = [{...viewData, 'module_name': module_name, 'api': `/${'embedding'}`, 'payload':{...dataSet}, 'response':[], 'error_details': err, 'status_code':'' }];
          if(Number(process.env.REACT_APP_DEBUG_MODE) === 1 || Number(sessionStorage.getItem('debugMode')) === 1){ 
            recordErrorAPIdata(localStorage.getItem('tenant_cname'), ...logData);
          }
        })
  ...
  const graphEmbed = (reportId, graphTitle) => {
    ...
    axios.get("/" + tenantCname + "/api/getreport", {
      headers: {
        "Accept": "application/JSON",
        "Authorization": "Bearer " + localStorage.getItem('token')
      },
      ...
      .then((res) => {
        ...
        logData = [{...viewData, 'module_name': module_name, 'api': `/${'embedding'}`, 'payload':{
        "report_id": reportId,
        "graph_only": 1,
        "graph_title": graphTitle
  ...
  return (
    <>
      ...
      <img className="modale_btn" id="ani" src={Group_75} alt="Group_75" onClick={embeddingModule} />
      <Drawer anchor="right" open={isDrawerOpen} onClose={closeDrawer} >
        <div className="drawer-content drawer-boxSize">
          <i onClick={closeDrawer} className="closeButton"><IoMdClose style={{ width: '24px', height: '24px' }} /></i>
          <div className="row">
            <div className="col-12 bnds">
              <div className="sortWrap">
                {/* <ul id="sortable"> */}
                  {(dash?.data || []).sort((a, b) => a.embedding_order - b.embedding_order).map((dashline, i) => (
                    ...
                    dashline.embedding_type === "Report" ?
                      ...
                      //   <li key={dashline.id} className={"ui-state-default chart-100p"} draggable={false}>
                          <div className="card m-3" key={i}>
                            <div className="card-header">{dashline?.embedding_name !== '' ? dashline?.embedding_name : 'Report Graph' }</div>
                      ...
                      : dashline.embedding_type === "Text" ?
                        ...
                        <div className="card m-3" key={i}>
                        <div className="card-header">{dashline?.embedding_name !== '' ? dashline?.embedding_name : 'Notes'}</div>
                              <div className="card-body text-center">
                        ...
                        : dashline.embedding_type === "IFrame" ?
                          ...
                          <div className="card m-3" key={i}>
                          <div className="card-header">{dashline?.embedding_name !== '' ? dashline?.embedding_name : 'IFrame' }</div>
                              <div className="card-body text-center">
                                {}
                                  <iframe style={{pointerEvents: 'none'}} src={(dashline?.embedding_value.indexOf('trimmedValue') !== -1) ? window.location.origin + '/#/home' + dashline?.embedding_value.split('trimmedValue')[1] : dashline?.embedding_value} height={400} width={'100%'} />
                                </div>
                               </div>
                          //   </li>
                          // </Draggable>
                          ...
                          <div className="card m-3" key={i}>
                          <div className="card-header">{dashline?.embedding_name !== '' ? dashline?.embedding_name : 'Image' }</div>

                          <div className="card-body text-center">
                                  <img src={dashline?.embedding_value} alt="" width="100%" height="" />
                                </div>
                                </div>
                            //   </li>
                            // </Draggable>
                                ...
                                <li key={dashline.id} className={"ui-state-default chart-100p"} draggable={false}>
                                  <div className="chartWrapper">
                                    <div id="mydiv0" className={"mydiv normalscreen"}>
                                      <div className="reziseDiv" style={{ "backgroundColor": "rgb(255, 255, 255)" }}>
                                        <div className="chartDraghead">{dashline.report_name}</div>
                                                    ...
                                                    (da.reportdetails.datasets || []).map((dts, k) => (
                                                      d === k ?
                                                        <tr key={k}>
                                                          ...
                                                          <td className="pl-1" data-title="Venue Code">{dts}</td>
                                                        </tr> : null
                                                    ))
                                                  ))
                                                ))
